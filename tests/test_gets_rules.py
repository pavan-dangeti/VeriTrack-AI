...
class TestViolationRule:
    def test_core_rule_fires(self):
        ...
        assert violation and "Customer Leave" in reason and "Company Leave" in reason
...
class TestNormalization:
    HEADERS = ["EMP CODE", " Emp Name ", "Company Email", "Personal Email",
               "Dept", " Customer Leave ", "acme leave"]

    def test_messy_headers_map_correctly(self):
        from app.services.extraction.normalize import build_header_map, has_required_columns

        assert has_required_columns(self.HEADERS)
        mapped = build_header_map(self.HEADERS)
        assert "customer_leave" in mapped and "company_leave" in mapped
    ...
    def test_row_normalization_with_extras(self):
        ...
        assert row.values["company_leave"] == ""
...
def test_company_leave_column_is_recognised_by_any_organisation_name():
    from app.services.extraction.normalize import build_header_map

    for header in ("Company Leave", "Acme Leave", "ACME Corp Leave"):
        m = build_header_map(["Employee ID", "Name", "Customer Leave", header, "Leave Type"])
        assert m["company_leave"] == header and m["customer_leave"] == "Customer Leave"
