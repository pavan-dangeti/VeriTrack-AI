...
GETS_A_CSV = """EMP CODE,Emp Name,Customer Leave,Company Leave
E-100,Ann Weaver,✓,
E-200,Bob Chen,X,✓
E-999,Ghost Person,✓,
   ,   ,✓ ,
...
GETS_B_CSV = """Employee ID,Name,Customer Leave,Company Leave
E-100,Ann Weaver,x,
...
class TestEndToEnd:
    async def test_full_flow(self, client, ma_user_id):
        ...
        repo_batch = r.json()
        assert repo_batch["status"] == "COMPLETED"
        ...
        # Unique violators: E-100 (in both files), E-300, and E-400 (customer
        # 'True' ticked, company 'No' parses as blank → violation).
        assert totals["violations"] == 3, analysis
        assert totals["emails_sent"] == 2  # E-100 official, E-400 personal
        ...
        assert by_code["E-400"]["email_to"] == "dia.patel@corp.io"
        ...
        async with __import__("tests.conftest", fromlist=["open_session"]).open_session() as db:
    ...
    async def test_reports_generated_and_downloadable(self, client, ma_user_id):
        ...
        xlsx = await client.get(f"/api/v1/runs-batch/{batch_id}/reports/3tab.xlsx",
                                headers=headers)
        # the route needs the run id, so fetch the analysis first
        analysis = (await client.get(
