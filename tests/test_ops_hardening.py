"""Observability, rate limiting, retention purge and the employee soft-delete guarantee."""
...
class TestReadiness:
    async def test_health_ready_probes_dependencies(self, client):
        ...
        assert "redis" not in checks  # not a dependency outside celery mode

    async def test_health_ready_probes_redis_in_celery_mode(self, client, monkeypatch):
        from app.core.config import settings

        monkeypatch.setattr(settings, "processing_mode", "celery")
        r = await client.get("/health/ready")
        assert r.status_code in (200, 503)
        assert isinstance(r.json()["checks"]["redis"], bool)
...
class TestRateLimiting:
    async def test_uploads_rate_limited_for_one_user(self, client, ma_user_id):
        ...
        allowed = int(UPLOAD_LIMIT.split("/")[0])
        statuses = []
        ...
        assert statuses[:allowed] == [201] * allowed
        assert statuses[allowed:] == [429, 429], statuses
