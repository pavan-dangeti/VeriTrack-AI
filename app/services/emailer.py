"""Outbound violation emails via Microsoft Graph sendMail (retries 429/5xx); dry-run by default."""
...
def _violation_email_body(v: ViolationResult) -> dict:
    ...
    return {
        "message": {
            ...
            "body": {
                "contentType": "Text",
                # Graph's itemBody field is `content` (bodyText is silently dropped)
                "content": (
                    f"Dear {name},\n\n"
                    f"Our records indicate a leave compliance issue:\n"
                    f"  {v.violation_reason}\n\n"
                    "Please apply the matching leave in the company leave system, or "
