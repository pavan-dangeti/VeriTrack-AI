...
async def _get_visible_employee_or_raise(db: AsyncSession, actor: User, employee_id):
    ...
    # O(1) scope check (never load the whole repository to test one id)
    if actor.role is UserRole.MANAGER:
        visible = employee.manager_id == actor.id
    elif actor.role is UserRole.HR:
        visible = actor.manager_id is not None and employee.manager_id == actor.manager_id
    else:
        visible = True
    if not visible:
