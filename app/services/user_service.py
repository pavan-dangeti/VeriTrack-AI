...
Role-creation authority and HR scoping live here, not in routers, so every caller gets the same rules.
"""
...
def scoped_users_query(actor: User):
    """Query-layer visibility scoping; every list endpoint must go through this.

    MA/EXEC see every user, MANAGER only the HR users they created, HR nobody.
    """
...
async def get_user_profile(
    ...
    """Profile drill-down: self always; MA/EXEC anyone; MANAGER their own HR users; HR only self."""
...
async def set_active(
    ...
    No self-disable, and the Master Admin can never be disabled; with the DB partial unique index
    this keeps exactly one active MA.
    """
    target = await db.get(User, target_id)
    if target is None:
        raise UserRuleError(404, "not_found", "User not found")
