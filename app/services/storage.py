"""Object storage (local disk / volume / S3-compatible). Keys are manager-scoped by construction."""
...
class LocalDiskBackend:
    """Filesystem backend: dev/test, or a persistent volume in production
...
def get_storage() -> StorageBackend:
    ...
    if _backend is None:
        if settings.storage_backend == "s3":
            _backend = S3Backend()
        elif settings.storage_backend == "volume":
            _backend = LocalDiskBackend(settings.local_storage_root)
        else:
            if settings.is_production:
                raise StorageError(
                    "ephemeral local storage must not be used in production; set "
                    "STORAGE_BACKEND=s3, or STORAGE_BACKEND=volume on a persistent disk"
                )
            _backend = LocalDiskBackend(settings.local_storage_root)
    return _backend
