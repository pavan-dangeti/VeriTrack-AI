"""Content inspection by magic bytes; the client's filename is never trusted."""
...
def sniff_content_type(data: bytes, declared_filename: str) -> str:
    """Returns the canonical content type or raises FileValidationError.

    Binary formats are identified purely by magic bytes; the extension is only a tiebreaker for CSV.
    """
    if len(data) < 8:
