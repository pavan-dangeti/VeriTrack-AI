...
def _flatten_rows(rows: list[dict], columns: list[str]) -> tuple[list[str], list[list]]:
    """Row dicts (+ their `extra` payload) → header + string cells; nested
    values (lists/dicts) are never exported as cells."""
    out = []
    for r in rows:
        record = dict(r or {})
        extras = record.pop("extra", {}) or {}
        merged = {**record, **extras}
        cells = []
        for c in columns:
            value = merged.get(c, "")
            if isinstance(value, (dict, list)) or value is None:
                value = ""
            cells.append(_escape_formula(str(value)))
        out.append(cells)
