"""Post-analysis reporting: summary PDF + 3-tab Excel, stored and emailed to the uploading Manager."""
...
async def generate_and_store_reports(db: AsyncSession, run: AnalysisRun) -> dict:
    ...
    xlsx_bytes = await asyncio.to_thread(build_excel_3tab, run, violations, skips)
    key_xlsx = build_key(
    ...
    await asyncio.to_thread(storage.put, key_xlsx, io.BytesIO(xlsx_bytes), len(xlsx_bytes))
    db.add(
        GeneratedReport(
    ...
    pdf_bytes = await asyncio.to_thread(build_summary_pdf, run, violations)
    key_pdf = build_key(
        manager_id=str(run.manager_id),
        kind="GETS",
        batch_id=str(run.batch_id),
    ...
    await asyncio.to_thread(storage.put, key_pdf, io.BytesIO(pdf_bytes), len(pdf_bytes))
    db.add(
        GeneratedReport(
