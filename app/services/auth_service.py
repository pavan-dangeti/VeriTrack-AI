...
async def authenticate_password(
    ...
    *,
    ...
    """Validates credentials, enforces lockout, audits every attempt.

    Unknown email and wrong password raise the same error so accounts cannot be enumerated.
    """
    ...
    if user is None:
        ...
        await asyncio.to_thread(hash_password, "timing-equalizer-dummy-password")
    ...
    # lockout state (there is no lockout-specific response).
    password_ok = user.auth_type is AuthType.PASSWORD and await asyncio.to_thread(
...
async def rotate_refresh(
    ...
    *,
    ...
    now = datetime.now(UTC)
    hash_ok = hash_refresh_token(raw_refresh_token) == row.token_hash
    # Two tabs refreshing with the same cookie race: the loser presents a token
    # its sibling rotated a moment ago. Inside the grace window that is not
    # theft, so it gets its own successor instead of killing every session.
    just_rotated = (
        row.revoked_at is not None
        and row.replaced_by_jti is not None
        and now - row.revoked_at <= REFRESH_REUSE_GRACE
    )
    if just_rotated:
        # ...unless the family was killed meanwhile (then no live token remains)
        live = await db.execute(
            select(RefreshToken.id).where(
                RefreshToken.user_id == row.user_id, RefreshToken.revoked_at.is_(None)
            ).limit(1)
        )
        just_rotated = live.first() is not None

    if not hash_ok or (row.revoked_at is not None and not just_rotated):
        await revoke_all_for_user(db, row.user_id)
        reason = "hash_mismatch" if not hash_ok else "reuse"
        await audit_service.record(
            db,
    ...
    await db.execute(
        ...
        .values(revoked_at=now, replaced_by_jti=uuid.UUID(new_jti))
    )
    db.add(
        RefreshToken(
            user_id=row.user_id,
            ...
            expires_at=row.expires_at if just_rotated else now + timedelta(days=settings.refresh_token_expire_days),
        )
    )
    access, _ = create_access_token(user_id=user.id, role=user.role.value)
    await db.commit()
