...
async def export_employees(actor: RepoViewer, db: DB):
    from fastapi.responses import Response

    from app.services.exporters import export_csv

    employees = await employee_service.list_scoped(db, actor)
    columns = ["employee_code", "full_name", "official_email", "personal_email", "department"]
    data = export_csv([_serialize(e) for e in employees], columns)
    return Response(
        content=data,
        media_type="text/csv",
