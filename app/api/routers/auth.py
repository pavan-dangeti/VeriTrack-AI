...
The refresh token is an HttpOnly, SameSite=Strict cookie scoped to /api/v1/auth. The CSRF cookie is
JS-readable on purpose (double-submit) and is enforced only on the cookie-based endpoints
(refresh/logout), so bearer-token API clients are unaffected.
"""
