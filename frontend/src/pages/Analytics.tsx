...
const MONTH_FMT = (m: string) =>
  new Date(`${m}-02T00:00:00Z`).toLocaleString(undefined, { month: "short", year: "numeric" });
...
export function AnalyticsPage() {
  ...
  const emailSeries = [
    { name: "Sent", value: emails.sent, color: EMAIL_COLORS[0] },
    { name: "Missing email", value: emails.missing, color: EMAIL_COLORS[1] },
    { name: "Errored", value: emails.errored, color: EMAIL_COLORS[2] },
    { name: "Skipped", value: emails.skipped, color: EMAIL_COLORS[3] },
  ].filter((e) => e.value > 0);
  const anyEmail = emailSeries.length > 0;
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    innerRadius={55}
                    outerRadius={85}
                    paddingAngle={emailSeries.length > 1 ? 2 : 0}
                  >
                    {emailSeries.map((e) => (
