/** Manager flow: upload GETS files → live progress → analyze → download reports (fetch mocked). */
...
const state = vi.hoisted(() => ({ batchCalls: 0, uploads: [] as string[] }));
...
vi.mock("../api/client", () => ({
  ...
  api: vi.fn((path: string, _init?: RequestInit) => {
    ...
    if (path.startsWith("/uploads/batches")) {
      state.batchCalls++;
...
describe("Manager GETS flow", () => {
  it("upload → per-file real statuses → analyze → export/download available", async () => {
    ...
    await waitFor(() => expect(state.uploads.length).toBeGreaterThan(0));
    expect(state.uploads[0]).toContain("/uploads?kind=GETS");
  ...
  it("PENDING_OCR file renders amber with reason, DONE file shows review count", async () => {
    ...
    const pending = await screen.findAllByText("Awaiting OCR");
